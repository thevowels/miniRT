...
/*   Updated: 2026/09/05 15:25:14 by aphyo-ht         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_camera(t_camera *cam, int width, int height)
{
	...
	cam->w = vec_norm(vec_cross(vup, cam->w));
	cam->v = vec_cross(cam->w, cam->u);
	theta = cam->fov * M_PI / 180.0;
	h = tan(theta / 2.0);
	vp_h = 2.0 * h;
	vp_w = vp_h * ((double)width / (double)height);
	cam->viewpoirt_u = vec_scale(cam->u, vp_w);
	cam->viewpoirt_v = vec_scale(cam->v, -vp_h);
	cam->pixel00_loc = vec_sub(cam->pixel00_loc, vec_scale(cam->viewpoirt_u, 0.5));
	cam->pixel00_loc = vec_sub(cam->pixel00_loc, vec_scale(cam->viewpoirt_v, 0.5));
}
...
t_ray	get_ray(const t_camera *cam, int x, int y, int width, int height)
{
	...
	sample_pixel = vec_add(cam->pixel00_loc, vec_scale(cam->viewpoirt_u, s));
	sample_pixel = vec_add(sample_pixel,vec_scale(cam->viewpoirt_v, t));
	ray.orig = cam->pos;
